export const Minesweeper = {
  id: "minesweeper",
  name: "扫雷（Minesweeper）",
  description: "经典 Windows 扫雷：左键翻开，右键/长按插旗，自动展开空白区域。",
  create({ canvas, ctx, ui, utils }){
    ...
    function layout(){
      ...
      cell = Math.max(14, Math.min(40, Math.min(maxCellByW, maxCellByH)));
    ...
    function placeMinesAvoid(safeX, safeY){
      // 首次点击保证安全：safe cell 以及周围 8 格都不放雷
    ...
    function checkWin(){
      ...
      if(!gameOver && revealedCount >= totalSafe){
        gameOver = true;
        win = true;
        timerRunning = false;
        // 自动插旗剩余雷
    ...
    function updateHud(forceEnd=false){
      ...
      const left = `💣 雷：${minesLeft}   🚩 旗：${flags}`;
      let right = `⏱️ 时间：${Math.floor(time)}s`;
      if(gameOver || forceEnd){
        right += win ? "   ✅ 胜利！" : "   💥 失败！";
      }else if(firstClick){
        right += "   点击开始";
      }
    ...
    function render(){
      ...
      const boardW = cols*cell;
      const boardH = rows*cell;
      utils.bevelFill(ctx, boardX-10, boardY-10, boardW+20, boardH+20, 18);
    ...
    function pointerDown(e){
      ...
      if(e.pointerType !== "mouse"){
        // touch/pen: long press to flag
        clearTimeout(lpTimer);
        lpTimer = setTimeout(()=>{
          lpFired = true;
          const c = getCellFromXY(e.x,e.y);
          if(c) toggleFlag(c.x,c.y);
        }, LONG_PRESS_MS);
      }else{
        // mouse left press does nothing until up (we allow press+drag)
      }
    }
    ...
    function pointerUp(e){
      ...
      if(e.pointerType === "mouse"){
        ...
        // touch: if long press fired, do not reveal
        if(lpFired) return;
        reveal(c.x,c.y);
      }
    }
    ...
    function getRulesHtml(){
      return `
        ...
        <ul>
          ...
          <li>移动端：<code>点击</code>翻开，<code>长按</code>插旗/取消（约 ${LONG_PRESS_MS}ms）。</li>
        </ul>
        <p><b>规则：</b></p>
        <ul>
